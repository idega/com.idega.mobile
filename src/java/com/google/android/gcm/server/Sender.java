...
public class Sender {
  ...
  public Result sendNoRetry(Message message, String registrationId)
    ...
    String endpoint = IWMainApplication.getDefaultIWMainApplication().getSettings().getProperty("android_push_endpoint", Constants.GCM_SEND_ENDPOINT);
    HttpURLConnection conn = post(endpoint, requestBody);
    int status = conn.getResponseCode();
    if (status == 503) {
      logger.warning("GCM service (" + endpoint + ") is unavailable. Status: " + status);
      return null;
    }
    if (status != 200) {
      throw new InvalidRequestException(status, "Error sending '" + requestBody + "' to " + endpoint);
    }
    try {
  ...
  public MulticastResult sendNoRetry(Message message,
    ...
    logger.fine("JSON request: " + requestBody);
    HttpURLConnection conn =
        post(IWMainApplication.getDefaultIWMainApplication().getSettings().getProperty("android_push_endpoint", Constants.GCM_SEND_ENDPOINT), "application/json", requestBody);
    int status = conn.getResponseCode();
    String responseBody;
    if (status != 200) {
      responseBody = conn == null || conn.getErrorStream() == null ? "No response" : getString(conn.getErrorStream());
      logger.warning("JSON error response: " + responseBody + ". Code: " + status);
